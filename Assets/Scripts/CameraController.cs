...
public class CameraController : MonoBehaviour {
    ...
    void Update() {
        ...
        if(Input.GetKey("d")) {
       	...
       	// Android support
		if(Input.GetMouseButtonDown(0) && !Stage.IsPointerOverUIObject()) {
		...
		if(Input.GetMouseButton(0) && !Stage.IsPointerOverUIObject() && (touchPrev.x != Input.mousePosition.x) && (touchPrev.y != Input.mousePosition.y)) {
        ...
        if(following != null) {
    		...
    		if(Vector2.Distance(v, new Vector2(pos.x, pos.y)) > 1.1f * Stage.s.unitSpeed * Time.deltaTime)
    			moving = true;
			else
    			moving = false;
        ...
        if(!Stage.IsPointerOverUIObject() && !Input.GetKey(KeyCode.LeftControl))
