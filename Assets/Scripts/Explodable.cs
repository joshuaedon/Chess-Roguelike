...
public class Explodable : MonoBehaviour {
    ...
    string fragmentLayer = "Fragments";

    public List<GameObject> fragments;
    private List<List<Vector2>> polygons = new List<List<Vector2>>();

    public void generateFragments(Vector2 velocity) {
        fragments = new List<GameObject>();
        fragments = GenerateTriangularPieces(gameObject, velocity, extraPoints);

        //sets additional aspects of the fragments
        foreach (GameObject p in fragments) {
            if (p != null) {
                p.layer = LayerMask.NameToLayer(fragmentLayer);
                p.GetComponent<Renderer>().sortingLayerName = fragmentLayer;
            }
        }
		Destroy(gameObject);
    }

    private static List<GameObject> GenerateTriangularPieces(GameObject source, Vector2 velocity, int extraPoints = 0, Material mat = null) {
        ...
        // Get rigidbody velocity if the gameObject has one
        Rigidbody2D regidbody2D = source.GetComponent<Rigidbody2D>();
        Vector2 origVelocity = (regidbody2D != null) ? regidbody2D.velocity : velocity;
        ...
        foreach (Triangle tri in voronoi.Triangles()) {
            ...
            foreach (List<Vector2> triangle in clippedTriangles) {
                pieces.Add(generateTriangularPiece(source, triangle, origVelocity + new Vector2(Random.Range(-1f, 1f), Random.Range(-1f, 1f)), origScale, origRotation, mat));
            }
        }
    ...
    private static GameObject generateTriangularPiece(GameObject source, List<Vector2> tri, Vector2 origVelocity, Vector3 origScale, Quaternion origRotation, Material mat) {
        ...
        GameObject piece = (GameObject)Instantiate(Resources.Load("prefabs/Fragment"), source.transform.parent);
        piece.transform.parent = source.transform.parent;
        piece.transform.localPosition = source.transform.localPosition;
        ...
        Rigidbody2D rigidbody = piece.GetComponent<Rigidbody2D>();
    ...
    private static List<Vector2> getRendererPoints(GameObject source) {
        ...
        points.Add(new Vector2(bounds.center.x - bounds.extents.x, bounds.center.y - bounds.extents.y) - (Vector2)source.transform.localPosition);
        points.Add(new Vector2(bounds.center.x + bounds.extents.x, bounds.center.y - bounds.extents.y) - (Vector2)source.transform.localPosition);
        points.Add(new Vector2(bounds.center.x + bounds.extents.x, bounds.center.y + bounds.extents.y) - (Vector2)source.transform.localPosition);
        points.Add(new Vector2(bounds.center.x - bounds.extents.x, bounds.center.y + bounds.extents.y) - (Vector2)source.transform.localPosition);
        return points;
    ...
    private static Rect getRect(GameObject source) {
		...
		rect.center = new Vector2(rect.center.x + bounds.center.x - source.transform.localPosition.x, rect.center.y + bounds.center.y - source.transform.localPosition.y);
		return rect;
    }
    ...
    private static Vector3 calcPivotCenterDiff(GameObject target) {
        ...
        Vector3 pivot = target.transform.InverseTransformPoint(target.transform.localPosition);
